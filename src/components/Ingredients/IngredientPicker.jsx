...
export default function IngredientPicker({ setIngredients, ingredients, data, diet, setDiet }) {
    ...
    const handleDietChange = (e) => {
        // Check if diet is already selected and remove it if it is
        if (diet.includes(e.target.value)) {
            const newDiet = diet.filter(diet => diet !== e.target.value)
            setDiet(newDiet)
        } else {
        ...
        <div className='relative'>
            <div className='flex w-full gap-2'>
                ...
                <div className='relative w-max flex'>
                    <div className='select-none cursor-pointer h-full items-center w-max pr-10 px-2 p-1 text-base flex text-[#FAEBD2] bg-[#7A835E] rounded-full' onClick={() => {setShowDietlist(!showDietlist)}}>
                        Diets (ex: vegan)
                        <svg className={"absolute top-1/2 right-3 -translate-y-1/2 origin-center"}  width="16" height="18" viewBox="0 0 82 46" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M5.302 5.24268L40.9799 40.9205L76.6577 5.24268" stroke="#FAEBD2" strokeWidth="8.91947" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                    ...
                    </div>
                    {
                        ...
                        <fieldset onChange={handleDietChange} className='flex flex-col absolute divide-y divide-[#7A835E] border-2 border-[#7A835E] h-fit pl-2 overflow-y-auto top-12 w-full rounded left-0 bg-[#FAEBD2]'>
                            <ul className='divide-y divide-[#7A835E]'>
                               {/* Checkboxes for each diet Spoonacular handles */}
                                <li className='flex gap-1'>
                                    <input type="checkbox" value="gluten-free" />
                                    <label htmlFor="gluten free">Gluten Free</label>
                                </li>
                                <li className='flex gap-1'>
                                    <input type="checkbox" value="ketogenic" />
                                ...
                                <li className='flex gap-1'>
                                    <input type="checkbox" value="paleo" />
                                    <label htmlFor="paleo">Paleo</label>
                                </li>
                                <li className='flex gap-1'>
                                    <input type="checkbox" value="primal" />
                                    <label htmlFor="primal">Primal</label>
                                </li>
                                <li className='flex gap-1'>
                                    <input type="checkbox" value="whole30" />
            ...
            {showDatalist && queryResults.length !== 0 && (
                <ul className='flex flex-col absolute divide-y divide-[#7A835E] border-2 border-[#7A835E] h-40 pl-2 overflow-y-auto top-12 w-4/5 rounded left-0 bg-[#FAEBD2]'>
                {showDatalist && queryResults.map(result => {
                    return (
                        <>
                        <li onClick={ () => {addIngredient(result.id) } } className='hover:bg-slate-300 py-1 cursor-pointer' key={result}  >
